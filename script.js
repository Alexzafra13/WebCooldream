...
if (!marquee || !marqueeContent || !loader) {
  console.error('Faltan elementos en el HTML: .marquee, .marquee-content o .loader');
}
  ...
  text.split('').forEach((char, index) => {
    if (char === ' ') {
      if (currentWord) {
        ...
        wordSpan.innerHTML = currentWord.split('').map(letter => `<span class="letter">${letter}</span>`).join('');
...
function handleMouseMove(e) {
  ...
  projectNames.forEach((name) => {
    ...
    letters.forEach((letter, index) => {
      const letterCenter = nameLeft + (index + 0.5) * (nameWidth / letters.length);
  ...
  elements.forEach((project) => {
    ...
    project.addEventListener('mouseenter', () => {
      ...
      if (videoSrc) {
        ...
        currentVideo = preloadVideosCache[videoSrc] || document.createElement('video');
        ...
        if (currentVideo.readyState >= 2) {
          currentVideo.play().catch(err => console.error('Error al reproducir:', err));
        } else {
          currentVideo.onloadeddata = () => currentVideo.play().catch(err => console.error('Error tras carga:', err));
        }
      }
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  preloadVideos(videoUrls, () => {
    ...
    if (marquee) {
      ...
      const clonedProjects = document.querySelectorAll('.marquee .marquee-content:last-child .project-name');
